...
4. ### 总结：
   ...
   大部分标签都可以当做文本以及图片标签使用。

   但是，一般请不要这么做；每个标签都有其自身的含义，尽量别乱用。

   之前的知识点，会不断地在后续的小练习中继续重复学习。所以，不用担心记不住，重点在于你要动手跟着练习。
   
   同时一些新的知识点，不会再单独列文档学习了——边做边学——做中学。
   
   下面的教程就开始做一些小练习，难度比学知识点要大——现在是在运用学过的知识点。
   
   **注意：**
   
   **任何效果的实现方式都有多种。**
   
   **不要仅仅跟着我的写，希望你还会动手去改写；而不是仅仅“复制”我的代码。**
